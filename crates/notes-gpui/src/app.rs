...
impl Main {
    pub fn new(
        ...
        let ancestors = service.get_ancestors(&current_note_path).unwrap();
    ...
    fn on_text_editor_event(
        ...
        cx.background_executor()
            .spawn(async move |cx| {
                let service = cx.global_mut::<NotesService>();
                let _ = service.save_note(&note_path, &content);
            })
            .detach();
...
mod tests {
    ...
    async fn test_dirty_flag_set_on_change(cx: &mut TestAppContext) {
        ...
        let (main, editor) = cx.update(|cx| {
            ...
            let main = cx.new(|cx| Main::new(text_editor.clone(), note_path.clone(), cx));
            (main, text_editor)
        });
